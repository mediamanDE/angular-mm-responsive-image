...
describe('MmResponsiveImageComponent', function () {
    ...
    describe('::ngOnChanges', function () {
        ...
        it('should take the smallest image as the ::fallbackSource if it was not set explicitly', function () {
            component.sources = sourcesMock;
            component.ngOnChanges();
            expect(component.fallbackSource).toBe(component.sources[3].src);
        });
        it('should not set the ::fallbackSource if there are no ::sources', function () {
            component.ngOnChanges();
            expect(component.fallbackSource).toBe('');
        });
        it('should use a polyfill in legacy browsers', function (done) {
            window['picturefill'] = jasmine.createSpy('picturefill');
            component.sources = sourcesMock;
            var fallbackEl = compiled.query(platform_browser_1.By.css('img')).nativeElement;
            var sourceEls = compiled.queryAll(platform_browser_1.By.css('source'));
